...
int    act_func = 4;           //1-Sigmoid, 2-Hyperbolic tangent, 3-Sine function, 4-Cosine function
double learning_rate = 0.15;   //The rate at which it learns
double momentum = 0.00;
map<vector<int>, int> translation;

double random()                //Return an random decimal b/n 0 and 1
{
...
public:
	...
	double f(double x) { if (act_func == 1) return 1.0 / (1.0 + exp(-x)); else if(act_func==2) return tanh(x); else if(act_func==3) return sin(x); else if(act_func==4) return cos(x); } //Sigmoid, hyperbolic tangent, sine, cosine
	double f_derv(double x) { if (act_func == 1) return f(x)*(1 - f(x)); else if(act_func==2) return 1 - pow(tanh(x), 2); else if(act_func==3) return cos(x); else if(act_func==4) return -sin(x);}
private:
	...
	for (int i = Neural_Net.size() - 2; i >= 0; --i)//Every Layer starting from the second last layer.
	{
		for (int j = 0; j<Neural_Net[i].size(); ++j)//Every Neuron in layer i even the bias neuron. The bias's weights still change
		{
			for (int k = 0; k<Neural_Net[i][j].weights.size(); ++k)//Every Weight
			{
				...
				{
					Neural_Net[i][j].gradients[k] = f_derv(Neural_Net[i + 1][k].get_input())
						* Neural_Net[i][j].get_output() * delta;
				}
				else
				{
					double sum_0 = 0.0;
