...
list<T>* append(list<T>* ls, T n)
{
	...
	if(new_ele==NULL)
	{
		cerr << "Error allocating in func append!\n";
		exit(1);
	}
	new_ele->val = n;
	new_ele->next = NULL;
...
int main()
{
	...
	ls=append<string>(ls, "hello");
	ls=append<string>(ls, "angel");
	ls=append<string>(ls, "badboy");
	ls=append<string>(ls, "aa");
	ls=append<string>(ls, "zodiac");
	ls=append<string>(ls, "killer");

	cout << "Unsorted: ";
	display(ls);
	
	bubble_sort<string>(ls);
